...
- Reconnect action execution is now an exclusive switch: DirectConnect and Retry reconnect and return without scheduling a scan; Scan, Stop, and DeferProbe are explicit; unknown actions use a documented reconnect default. Initial disconnect returns Retry before cache selection. `cargo test`: 275 passed; full `idf.py build`: PASS.
