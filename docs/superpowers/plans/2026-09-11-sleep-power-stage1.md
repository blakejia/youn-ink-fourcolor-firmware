...
**实测记录（2026-09-11 22:55 → 2026-09-12 00:02）**

设备最终停在提交版本（还原版 `xiaozhi.bin`，hash 校验通过）；插电即永不深睡。

| 观测 | 数值 | 判据 |
|---|---|---|
| 插电（还原版） | `Stay awake (60000 ms)` 反复，无 `Deep sleep` | 第 1 条 ✓（60000 = mains 常量 ⇒ `mains=1`） |
| 冷启动顺序 | bring-up 簇 t=5.7/10.7/15.7/20.7s，WiFi 连接 t=29.5s，首次绘屏簇 t=35.8s | 第 7 条 ✓（初始化簇早于 WiFi 与首次刷新） |
| 排期驱动绘屏 | `schedule updated: 2 pages` → `show page 1/2 md5="36be550c"` | 服务端权威索引打通 ✓ |
| 唤醒拉通知 | `Notify: notify "…" displaying`（22:55:32） | 第 5a 条 ✓（插电启动内） |
| **占空比**（测试构建，`mains` 伪造为 0） | 睡 598s / 醒 8.2s（排期未变）→ 睡 597s / 醒 16.0s（需绘屏） | 第 2 条 ✓ + 第 3 条部分 ✓（两个清醒窗长度区分"要不要画"） |
| **轮询节奏**（网络层） | `pages/schedule` 每次唤醒 +1（基线累计 8463 ≈ 旧固件 10 秒轮询一天的量）；`notifications/next` 每次唤醒 +1 | 60× 降幅 ✓，D4 ✓ |
| 未插电半边极性 | 未直接观测 | 见下 |

**未能在本硬件上观测**（USB 控制台与深睡互斥，且 attach 会复位设备 —— 见 managed skill `esp32s3-usbserial-flash-and-observe-traps`）：

- 第 4 条的**未插电半边**（`pin2` 应为 1、`mains` 应为 0）：仅有间接支持——同一决策代码在 `mains=0` 时入睡、在 `mains=1`（插电）时保持清醒，且 `power_present` 与 `pin2` 用的是同一个比较。
- 第 6 条（BOOT 唤醒）：ext0 在每次睡前布防，但"按键确实唤醒"需要一次人眼/按键确认；`wakeup cause=2` 的取值本身可静态核对。
- 第 8 条（插电触发提升）：测试构建把 `mains` 伪造为 0，该触发路径不可达；按键触发走同一段代码。
- 第 3 条的**人眼确认**：面板是否在约 10 分钟翻页时闪一次（睡眠与清醒在面板上不可区分，只能证"在轮换"）。
...
- **修法（2026-09-12 已做并真机验证）**：交互冷启动时把 UI 外壳那一次首绘**压住**（`Application::ui_boot_paint_deferred_`，仅当 `server_pairing_init() == SERVER_PAIR_OK` 时置位），让首次循环里画板那一帧成为唯一落到玻璃上的帧；`RunPowerCycle` 在 `page_sync_paint_if_changed()` 之后清标志，任何按键活动也会清（`NoteButtonActivity`），所以 UI 不可能被卡在"永不绘制"。
  **实测（同一台设备，插桩构建）**：`reason=ui` 请求 3 → **0**，`Performing FULL refresh` 2 → **1**，首次刷新时刻 t=28.2s → 34.6s（用户早约 18 秒看到页面），120 秒观测内计数稳定为 1；画板仍正常绘屏（`show page 1/2 md5="36be550c"`）。
  **为什么不加回退**：四类冷启动情形里丢掉 UI 首绘都是对的——同步成功且页变了 → 只有画板一帧；同步成功但玻璃上已是该页（画板会跳过且**不置 `DISPLAYING`**，`page_sync.rs:579`）→ 保留原页；0 页 → 画板画空页提示；同步失败 → 保留原图，正是 §13「失败退避期间保住画面」。原设计里"没在显示就补画 UI"的回退会把用户那页盖成设置外壳，已避免。

**实测记录（2026-09-11 22:55-22:57，插电态）**：启动日志已证 1 / 5a / 7 三条，判据与观测时间戳如下表「实测」列。

> ⚠️ **`[REFRESH] reason=…` 在本构建下不会打印**：`custom_lcd_display.cc:45` 有
> `#undef ESP_LOGI` + 重定义为空，该文件里**所有** INFO 日志（含本次重构前加的 4 条刷新归因标记）都被编译掉。
> 因此判据改用面板路径上真正会输出的两类行：`CustomLcdDisplay: EPD busy wait`（ESP_LOGW，一轮面板刷新一个 5/10/15 秒簇）与
> `RawDrawUiManager: Display refresh idle; input unlocked`（另一文件，正常打印）。
> 这也意味着**早前那次"刷新归因"没有产出任何设备侧证据**——它想回答的"同一按键两轮全刷是谁触发的"至今未答。
> 后续可选项（未做）：把那 4 条改成 `ESP_LOGW`，或去掉该文件的 `#undef`（会一并放出该文件其余 INFO，量需评估）。

| # | 场景 | 判据（只用实际会打印的标记；`Boot path:` 与 wake cause 由 T6 提供） | 实测 |
|---|---|---|---|
| 1 | 插电 | 无 `Deep sleep` 行，反复 `Stay awake (60000 ms)`（60000 = mains 常量，即 mains=1） | ✅ 22:55:31 与 22:56:31 各一次 60s 重装，无休眠行 |
| 2 | 拔电后静默周期（未到翻页点） | `Boot path: quiet (wakeup cause=1)`；本 boot 无 `[REFRESH]`；以 `Deep sleep N s (mains=0 sync_ok=1 pin2=1)` 结束 |
| 3 | 到翻页点的唤醒 | 该唤醒**恰好一个** `EPD busy wait` 簇，且紧随一次 `Display refresh idle` | — |
...
| 5 | 唤醒时存在待确认通知 | `Notify: notify "…" displaying`；按 UP 后 `Notify: ack ok`，服务端 `status=acked` | ✅ 前半（拉取+上屏）22:55:32；ack 待按键 |
| 6 | 按 BOOT 唤醒 | `Boot path: interactive (wakeup cause=2)`；随后 `Stay awake (…)` 直到宽限过期 |
| 7 | 冷启动（拔电重插） | `EPD busy wait` 簇出现在首次刷新与 WiFi 活动**之前**（证明跑的是 BringUpPanel 自己的序列，而非刷新循环的 FULL 路径） | ✅ t=5.7/10.7/15.7s 完成 bring-up 簇，t=20.3s 应用初始化、t=25.4s WiFi 连接、t=35.8s 才出现绘屏的那一轮簇；`Boot path: interactive (wakeup cause=0)` 与冷启动相符 |
