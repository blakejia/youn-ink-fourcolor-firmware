...
**断网也要能睡（2026-09-11 补充）**：`CanEnterSleepMode()` 原先只允许 `SyncIdle` 入睡，而 WiFi 连不上时生命周期停在 `WifiConnecting`，于是策略永远拿到 `busy=1`、以 15 秒的节拍保持清醒——一个已配对的设备进入 WiFi 盲区就会一直醒着把电耗光。规则改为：`SyncIdle` 允许入睡；**`WifiConnecting` 且本 boot 已尝试过同步且失败**也允许入睡（周期已试过、网络不可达，正是"下次定时再试"的占空比语义）；配网（`ApProvision`）与配对（`PairStart`/`WaitCode`/`ClaimPolling`）仍然排除——那些流程需要用户和屏幕。插电时不受影响，因为策略在 `mains` 分支就短路了。
...
- **duty cycle 生效**：测试构建（`mains` 伪造为 0）实测 睡 598s / 醒 8.2s（排期未变）、睡 597s / 醒 16.0s（需绘屏）；睡眠时长 ≈ `min(seconds_until_next_page, cap 600s)`，两个清醒窗长度区分"要不要画"。
- **轮询降幅 60×**：`/api/pages/schedule` 由每次唤醒 1 次取代每 10 秒 1 次（基线累计 8463 ≈ 旧固件一天）；`/api/notifications/next` 每次唤醒 1 次（D4）。
- **插电不睡**：还原版固件插电时反复 `Stay awake (60000 ms)`、无 `Deep sleep`。
- **冷启动顺序**：面板初始化簇（5/10/15/20s）早于 WiFi 连接（29.5s）与首次绘屏簇（35.8s），证明惰性 bring-up 的序列确实在跑。
- 未能在本硬件观测：未插电半边极性、BOOT 唤醒、插电触发的提升（原因：USB 控制台与深睡互斥，attach 会复位设备）。详见计划矩阵表。
- 硬件观察：插电时充电指示灯始终不亮（充电 IC 未上报 charging/full），与固件无关。
