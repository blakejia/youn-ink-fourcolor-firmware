...
- 插电也采样（修订 2026-09-22，经用户批准）：充电态只在插电时存在，必须
  经 `c` 显式上报；充电期电压 3.7→4.2 V 爬升本身就是有价值曲线（原「插电
  电压近恒值」判断对充电期不成立）。`kNoBattery`（无电池）不产样本。
...
| 插电（mains）| 照常采样，`c`=2/3 显式上报充电/充满（2026-09-22 修订）；`kNoBattery` 不产样本 |
| 越界值 | 服务端整点丢弃 |
